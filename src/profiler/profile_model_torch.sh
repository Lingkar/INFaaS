...
### PYTORCH
  ...
  rm -rf /tmp/model/
  mkdir -p /tmp/model/${par_mod}
  cp ${FROZEN_MODEL} /tmp/model/${par_mod}
  ...
  docker run -d --rm -p 8501:8501 \
    ...
    janleo500/infaaspytorch:latest \
  ...
  ready_state=""
  while [[ -z "${ready_state}" ]]; do
    python3 pytorch_query.py resized_img.jpg 8501 1 > /dev/null && ready_state="ready"
